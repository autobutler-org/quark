...
// TestCreateUser_Endpoint drives POST /admin/users: an account with a private
// folder comes back 201 as an active user and announces both the account and
// the folder; a taken name, an existing folder and an invalid name are refused
// with the status the app keys its copy off.
func TestCreateUser_Endpoint(t *testing.T) {
	...
	if info, err := os.Stat(filepath.Join(filesDir, "bob")); err != nil || !info.IsDir() {
	...
	if path, ok := kinds[eventbus.EventNewFolder]; !ok || path != "bob" {
		t.Errorf("new_folder path = %q (seen %v), want bob", path, ok)
	}
	...
	if err := os.Mkdir(filepath.Join(filesDir, "family"), 0o755); err != nil {
		t.Fatal(err)
	}
