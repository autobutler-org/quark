...
func newEngine() (*gin.Engine, error) {
	...
	// gin.Default, but with an access log and a panic log that redact ?token=
	// and ?downloadToken= (#2152).
	router := gin.New()
	router.Use(serverutil.AccessLogger(), serverutil.Recovery(gin.DefaultErrorWriter))
