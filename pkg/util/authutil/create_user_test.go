...
func TestCreateUser_PhraseOnFirstLoginOnly(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(f.filesDir, "bob")); !os.IsNotExist(err) {
...
// TestCreateUser_PrivateFolder checks the folder is made and owned by the new
// account through the access layer, and nobody else's.
func TestCreateUser_PrivateFolder(t *testing.T) {
	...
	if result.FolderPath != "bob" {
		t.Errorf("FolderPath = %q, want bob", result.FolderPath)
	}
	if info, err := os.Stat(filepath.Join(f.filesDir, "bob")); err != nil || !info.IsDir() {
	...
	if check := loaded.Access.Check("", "bob/notes.txt", accessutil.Owner); !check.Allowed {
		t.Errorf("new account's access to its folder = %+v, want owner", check)
	}
	if check := loaded.Access.Check("", "admin", accessutil.Read); check.Readable {
		t.Error("new account can read outside its folder")
	}
}
...
func TestCreateUser_RefusalsLeaveNothing(t *testing.T) {
	f := newCreateUserFixture(t)
	existing := filepath.Join(f.filesDir, "family")
	...
	if _, err := os.Stat(filepath.Join(f.filesDir, "admin")); !os.IsNotExist(err) {
