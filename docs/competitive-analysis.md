...
3. **Share links.** Send a file or folder to someone without an account, with an expiry and optionally a
...
4. **SMB (and probably WebDAV).** Every competitor mounts as a network drive in Finder and Explorer.
...
5. **Two-factor authentication on login.** Password plus recovery phrase is all we have. TOTP appears in
...
6. **A desktop sync client.** A folder on the laptop that mirrors to the Quark. Synology Drive, QNAP's
   Qsync, UGREEN's Sync & Backup desktop tools. Without it, Quark is a destination you visit rather than
   a place your files live.
7. **RAID or some redundancy story.** We manage devices and can run a snapshot backup to a second one,
...
8. **Per-folder permissions and quotas.** Follows directly from multi-user — shared folders that some
   accounts can read and others cannot. Not useful before item 1, essential immediately after.
9. **Photo intelligence: faces, objects, places.** We have EXIF, favorites, albums and perceptual-hash
...
Trash was on this list when the analysis was drafted and has since shipped — `storageutil` already had
`TrashFiles`, `ListTrash` and 30-day retention, and it now has handlers under
`internal/server/api/v0/trash/` and a `/trash` route ([#1814](https://github.com/autobutler-org/quark/issues/1814)).
...
| Trash / recycle bin with restore | Yes | Yes | **Yes** — 30-day retention, restore and empty (JN-TR-001 – 008) |
...
1. **Trust is the product.** QNAP's ransomware history and Synology's lock-in reversal did more damage
   ...
   or multi-user carelessly. Item 5 on the list above (2FA) is where this bites first.
