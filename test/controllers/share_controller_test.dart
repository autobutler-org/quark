...
void main() {
  ...
  test('loads the access, and who the item can be shared with', () async {
    ...
    // Groups first, everyone first among them. ada, signed in and owning
    // the item, is left out: her row is locked.
    expect(c.principals, const [everyone, family, bob]);
  ...
  test('an admin locks nothing', () async {
    ...
    expect(c.principals, contains(ada), reason: 'an admin can pick itself');
  });

  test(
    'taking away ownership set on the item is flagged, whatever does it',
    () async {
      final c = controller(isAdmin: true);
      await c.load();

      expect(c.losesOwnership(ada, AccessLevel.write), isTrue);
      expect(c.losesOwnership(ada, null), isTrue, reason: 'removing access');
      expect(
        c.losesOwnership(ada, AccessLevel.owner),
        isFalse,
        reason: 'granting owner again changes nothing',
      );
      expect(c.losesOwnership(bob, AccessLevel.read), isFalse);
      expect(
        c.losesOwnership(bob, null),
        isFalse,
        reason: 'bob owns only through Photos',
      );
    },
  );
  ...
  test(
    ...
    () async {
      ...
      expect(c.lockedKeys, isEmpty);
      expect(c.principals, contains(bob));
    },
  );
