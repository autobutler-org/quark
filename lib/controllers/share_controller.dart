...
class ShareController extends ChangeNotifier {
  ...
  /// accounts. A principal in [lockedKeys] is left out, so the share form
  /// can't change what its locked row can't.
  List<PrincipalItem> get principals {
    final locked = lockedKeys;
    return [
  ...
  /// such change, whichever control makes it.
  bool losesOwnership(PrincipalItem principal, AccessLevel? level) =>
      directLevel(principal) == AccessLevel.owner && level != AccessLevel.owner;
